...
\normalsize Jaime Iriarte (000000)
\end{tabular}
...
\noindent\textbf{Contributors:} Mirko Pica (65\%), Christian Prendin (10\%), Leonardo Grossi (25\%)
\vspace{0.1cm}
...
The training notebook extracts spectral descriptors and time-domain acoustic audio parameters. A lightweight Artificial Neural Network (ANN) model is built in TensorFlow/Keras to perform 5-class context classification. The pipeline applies pruning and post-training 8-bit integer quantization, exporting an optimized \texttt{.tflite} flatbuffer. We subsequently used the STM32Cube.AI (X-CUBE-AI) expansion pack to convert the TFLite flatbuffer model into optimized C code for embedded microcontroller execution.

\subsection{Embedded Inference Firmware Deployment}
The deployment firmware drops logging tasks and integrates the TinyML runtime engine. Real-time audio and light sensor buffers are passed through on-chip feature extraction algorithms. The structured features are subsequently pushed to the local inference loop at fixed intervals to determine environment classification weights.
...
if (inference_ready_flag) {
  ...
  ai_network_run(network_instance, &ai_input[0], &ai_output[0]); // Run AI network
...
\noindent\textbf{Contributors:} Leonardo Grossi (40\%), Christian Prendin (60\%)
\vspace{0.1cm}

To enhance the smart glasses platform with consumer-oriented functionalities, a lightweight TinyML model and a companion mobile application were developed.

The neural network performs environmental recognition directly on the glasses, using feature vectors extracted from 9 consecutive acquisition packets to reduce computational complexity and memory. The 39-dimensional feature set includes light intensity, spectral ratios, darkness level, flicker frequency distribution, audio energy, silence ratio, zero-crossing rate, and frequency-band energy. The classifier architecture consists of a multilayer perceptron with 39 input features, two hidden layers of 32 and 16 neurons, and a Softmax output layer corresponding to five environmental classes: Underground, Outdoor, Indoor Quiet, Indoor Normal, and Covered/Dark. The model was quantized to int8 and prepared for deployment on the STM32 microcontroller using X-CUBE-AI.

The environmental classifications are transmitted via BLE to a companion smartphone application developed with Flutter. Starting from the default Flutter example template, the application was extended to include BLE integration, custom packet parsing, real-time telemetry graphing, and dynamic user interface state adaptation. Instead of streaming raw sensor data, the application receives compressed context tokens, which are processed in real time by a local state management provider to dynamically adapt the user interface according to the detected environment.
...
The final integrated prototype successfully demonstrates real-time edge context classification using fused multimodal data streams. The trained 8-bit quantized Artificial Neural Network (ANN) achieved a macro $F_1$-score of 0.990 across the five target environments during validation testing, performing reliably within the strict memory constraints of the STM32U575 microcontroller. 

The corresponding classification confusion matrix is illustrated in Figure~\ref{fig:tinyml_confusion}. The system achieved its highest individual recall in isolating the "Covered/Dark" class (1.00) and the "Outdoor" class (1.00), owing to the distinct, wide-band spectral amplitude separation provided by the AS7341 multispectral light sensor. The primary analytical bottleneck emerged from localized acoustic similarities between the "Indoor Quiet" and "Indoor Normal" resting baselines. 

It is important to note that the training and validation dataset is relatively small, collected under restricted environmental conditions with a limited number of unique recording sessions. Consequently, the near-perfect accuracy and macro $F_1$-score (~99\%) obtained during validation are likely overoptimistic and do not represent the system's generalization performance in unstructured, real-world deployment scenarios.

\subsection{Short Video Demonstration}
A comprehensive 3-minute video demonstration showcasing the operational smart glasses hardware, local TinyML inference processing, and smartphone app synchronization is available at: \\
\url{www.google.com}
...
% Figure 3: Stack-up Layering
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Layer Stack.png}
    \caption{Four-layer PCB stack-up used for the LUMOS probe implementation.}
    \label{fig:stackup}
\end{figure}
